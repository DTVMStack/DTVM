...
echo "=== Pass 1: branch (HEAD 5357578) ===" | tee -a progress.log
date | tee -a progress.log
taskset -c 2 "$EVMONE_BENCH" \
  "${BRANCH_LIB},mode=multipass" \
  "$BENCHES" \
  --benchmark_filter="$FILTER" \
  --benchmark_repetitions=$REPS \
  --benchmark_format=json \
  --benchmark_out=branch.json > branch.stdout 2> branch.stderr
...
date | tee -a progress.log
taskset -c 2 "$EVMONE_BENCH" \
  "${BASELINE_LIB},mode=multipass" \
  "$BENCHES" \
  --benchmark_filter="$FILTER" \
  --benchmark_repetitions=$REPS \
  --benchmark_format=json \
  --benchmark_out=baseline.json > baseline.stdout 2> baseline.stderr
...
taskset -c 2 "$EVMONE_BENCH" \
  ...
  --benchmark_repetitions=$REPS \
  --benchmark_format=json \
  --benchmark_out=baseline_pingpong.json > baseline_pingpong.stdout 2> baseline_pingpong.stderr
