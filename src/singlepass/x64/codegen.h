...
#else
      // For multi-value, Op might be empty if values were already moved to
      // registers by handleReturnMultiValueImpl
      if (Op.isNone()) {
        // Values already in registers, skip the move
      } else {
        ZEN_ASSERT(Layout.getReturnType(0) == Op.getType());
#endif
      switch (Op.getType()) {
...
Operand handleCompareOpImpl(Operand LHS, Operand RHS) {
  ...
  if (!Exchanged) {
    ...
    constexpr CompareOperator ExchangedOpr = getExchangedCompareOperator<Opr>();
...
Operand handleLoadImpl(Operand Base, uint32_t Offset, uint32_t Align) {
  ...
  Addr = Base.isImm() ? asmjit::x86::Mem(ABI.getMemoryBaseReg(), Offset,
...
void handleBranchTableImpl(Operand Index,
  ...
  _ jmp(asmjit::x86::Mem(JmpReg, IndexReg, sizeof(uintptr_t) == 4 ? 2 : 3, 0));
...
std::vector<Operand>
handleCallMultiValueImpl(uint32_t FuncIdx, uintptr_t Target, bool IsImport,
                         bool FarCall, const ArgumentInfo &ArgInfo,
                         const std::vector<Operand> &Args) {
  // For multi-value returns, we use a similar approach to single value
  // but return multiple operands
  uint32_t NumReturns = ArgInfo.getNumReturns();
  std::vector<Operand> Results;

  // Call the function
  Operand PrimaryResult = emitCall(
      ArgInfo, Args, [this] { saveGasVal(); },
      [&]() {
#ifdef ZEN_ENABLE_DWASM
        if (IsImport) {
      ...
      [this, IsImport, &Results, NumReturns, &ArgInfo]() {
        loadGasVal();
        checkCallException(IsImport);
        ...
        if (NumReturns > 0) {
          ...
          // First result is in return register
          if (NumReturns >= 1) {
            Results.push_back(getReturnRegOperand(RetTypes[0]));
          }
          // Second integer result is in RDX, second FP in XMM1
          if (NumReturns >= 2) {
            if (RetTypes[1] == WASMType::I32 || RetTypes[1] == WASMType::I64) {
              Results.push_back(Operand(RetTypes[1],
                                        ABI.template getParamRegNum<I64, 1>(),
                                        Operand::FLAG_NONE));
            } else {
              Results.push_back(Operand(RetTypes[1],
                                        ABI.template getParamRegNum<F64, 1>(),
                                        Operand::FLAG_NONE));
            }
          }
          // Additional results would need stack-based return
          // For now, we handle up to 2 results
          for (uint32_t I = 2; I < NumReturns; ++I) {
            // TODO: Handle additional results from stack
            Results.push_back(getTempStackOperand(RetTypes[I]));
          }
        }
      });

  // If we have results from the callback, use those; otherwise use primary
  if (Results.empty() && NumReturns > 0) {
...
Operand handleCallIndirectImpl(uint32_t TypeIdx, Operand Callee,
  ...
  return emitCall(
      ...
      [this, NumHostAPIs, TypeIdx, Callee, TblIdx]() {
        ...
        asmjit::x86::Mem FuncPtrAddr(InstReg, FuncIdx, Shift,
...
#ifdef ZEN_ENABLE_WASI_MULTI_VALUE
// Multi-value call indirect implementation
std::vector<Operand>
handleCallIndirectMultiValueImpl(uint32_t TypeIdx, Operand Callee,
                                 uint32_t TblIdx, const ArgumentInfo &ArgInfo,
                                 const std::vector<Operand> &Arg) {
  uint32_t NumReturns = ArgInfo.getNumReturns();
  std::vector<Operand> Results;
  ...
  Operand PrimaryResult = emitCall(
      ...
      [&]() { _ call(ABI.getCallTargetReg()); },
      [this, &Results, NumReturns, &ArgInfo]() {
        loadGasVal();
        checkCallIndirectException();
...
#ifdef ZEN_ENABLE_WASI_MULTI_VALUE
void handleReturnMultiValueImpl(const std::vector<Operand> &Ops) {
  // For multi-value returns, we need to handle multiple return values
  // ABI for multiple returns:
  // - First integer result: RAX
  // - Second integer result: RDX
  // - First FP result: XMM0
  // - Second FP result: XMM1
  // - Additional results: passed via stack (return buffer)
  // NOTE: Currently only supports up to 2 results of each type
  if (Ops.empty()) {
    emitEpilog(Operand());
    return;
  }
  ...
  // For multiple return values, use registers for first 2 of each type
  uint32_t GpRegIdx = 0;
  uint32_t FpRegIdx = 0;
  for (uint32_t I = 0; I < Ops.size(); ++I) {
    const Operand &Op = Ops[I];
    WASMType Type = Op.getType();
    if (Type == WASMType::I32) {
      if (GpRegIdx == 0) {
        // First I32 result goes to EAX
        mov<I32>(ABI.template getRetRegNum<I32>(), Op);
        GpRegIdx++;
      } else if (GpRegIdx == 1) {
        // Second I32 result goes to EDX
        mov<I32>(ABI.template getParamRegNum<I32, 1>(), Op);
        GpRegIdx++;
      }
      // Additional I32 results are not supported - skip
    } else if (Type == WASMType::I64) {
      if (GpRegIdx == 0) {
        // First I64 result goes to RAX
        mov<I64>(ABI.template getRetRegNum<I64>(), Op);
        GpRegIdx++;
      } else if (GpRegIdx == 1) {
        // Second I64 result goes to RDX
        mov<I64>(ABI.template getParamRegNum<I64, 1>(), Op);
        GpRegIdx++;
      }
      // Additional I64 results are not supported - skip
    } else if (Type == WASMType::F32) {
      if (FpRegIdx == 0) {
        // First F32 result goes to XMM0
        mov<F32>(ABI.template getRetRegNum<F32>(), Op);
        FpRegIdx++;
      } else if (FpRegIdx == 1) {
        // Second F32 result goes to XMM1
        mov<F32>(ABI.template getParamRegNum<F32, 1>(), Op);
        FpRegIdx++;
      }
      // Additional F32 results are not supported - skip
    } else if (Type == WASMType::F64) {
      if (FpRegIdx == 0) {
        // First F64 result goes to XMM0
        mov<F64>(ABI.template getRetRegNum<F64>(), Op);
        FpRegIdx++;
      } else if (FpRegIdx == 1) {
        // Second F64 result goes to XMM1
        mov<F64>(ABI.template getParamRegNum<F64, 1>(), Op);
        FpRegIdx++;
      }
      // Additional F64 results are not supported - skip
    }
  }
  emitEpilog(Operand());
}
...
// unreachable
void handleUnreachableImpl() { _ jmp(getExceptLabel(ErrorCode::Unreachable)); }
...
Operand handleMemoryGrowImpl(Operand Op) {
  ...
  if (!Initialized) {
    SigBuf.NumParams = 1;
    SigBuf.NumParamCells = 1;
    SigBuf.NumReturns = 1;
    SigBuf.NumReturnCells = 1;
#ifdef ZEN_ENABLE_WASI_MULTI_VALUE
    SigBuf.ReturnTypesVec[0] = WASMType::I32;
#else
        SigBuf.ReturnTypes[0] = WASMType::I32;
#endif
    SigBuf.ParamTypesVec[0] = WASMType::I32;
    SigBuf.SmallestTypeIdx = uint32_t(-1);
    Initialized = true;
  }
  ...
  return emitCall(
      ...
      [this]() {
        ...
        auto InstReg = ABI.getModuleInstReg();
        _ mov(
            ABI.getMemorySizeReg(),
            asmjit::x86::Mem(InstReg, Ctx->Mod->getLayout().MemorySizeOffset));
        _ mov(
            ABI.getMemoryBaseReg(),
            asmjit::x86::Mem(InstReg, Ctx->Mod->getLayout().MemoryBaseOffset));
        _ bind(CallFail);
      },
      [] {});
}
...
Operand selectWithCMov(Operand Cond, Operand LHS, Operand RHS) {
  ...
  if (Layout.hasAvailTempReg<X64Type>(RetReg)) {
    ...
    ASM.mov<X64Type>(Ret.getMem<X64Type>(), X64Reg::getRegRef<X64Type>(ResReg));
  }
  return Ret;
}
...
Operand selectWithIf(Operand Cond, Operand LHS, Operand RHS) {
  ...
  mov<X64Type, ScopedTempReg0>(Ret, Operand(Type, RegNum, Operand::FLAG_NONE));
  return Ret;
}
